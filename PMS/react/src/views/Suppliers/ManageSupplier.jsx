...
import EditSupplierModal from "../common/Modals/EditSupplierModal.jsx";
import AddSupplierModal from "../common/Modals/AddSupplierModal.jsx";
...
const ManageInvoice = () => {
    ...
    const handleDelete = async ({ id }) => {
        try {
            ...
            const { data: resData } = await deleteSupplier(id);
            const msg = resData.message;
            setMessage({
                type: "success",
                body: msg || `Supplier with id ${id} deleted successfully`,
            });
        } catch (ex) {
            console.log(ex);
        }
    ...
    const columns = [
        ...
        {
            key: "address",
            name: t("supplier.address"),
            selector: (row) => row.address,
            sortable: true,
        },
        {
            key: "action",
            name: t("supplier.action"),
            sortable: false,
    ...
    return (
        <div className="main-container mt-24 md:m-10 p-12 md:p-10 bg-white rounded-3xl shadow-md dark:text-gray-200 dark:bg-secondary-dark-bg dark:[#484B52]">
            <Header title={t("supplier.manageSuppliers")} />
            <style>
                {`
                ...
                <div className="flex flex-wrap justify-between mb-4 gap-5">
                    ...
                    <div className="flex justify-center items-center gap-1">
                        ...
                        <PDFDownloadLink
                            ...
                            fileName={`suppliers-${new Date().toLocaleDateString()}.pdf`}
                        ...
                        <CSVLink
                            data={csvData}
                            filename={`suppliers-${new Date().toLocaleDateString()}.csv`}
                            className="text-xl hover:bg-blue-500 bg-blue-600 text-white active:bg-yellow-600 font-bold uppercase  px-3 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none  ease-linear transition-all duration-150"
                        >
                            <FaFileCsv />
                        </CSVLink>
